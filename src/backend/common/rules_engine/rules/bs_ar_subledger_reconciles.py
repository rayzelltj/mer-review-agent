...
from ..config import ArSubledgerReconcilesRuleConfig
...
@register_rule
class BS_AR_SUBLEDGER_RECONCILES(Rule):
    ...
    sources = ["QBO"]
    config_model = ArSubledgerReconcilesRuleConfig

    def evaluate(self, ctx: RuleContext) -> RuleResult:
        cfg = ctx.client_config.get_rule_config(self.rule_id, ArSubledgerReconcilesRuleConfig)
        if not cfg.enabled:
            return RuleResult(
        ...
        total_matches = [
            ...
            if self._is_total_ar(acct.name or "")
        ]
        if len(total_matches) > 1:
            return RuleResult(
                ...
                summary=(
                    f"Multiple AR total lines found in Balance Sheet as of {ctx.period_end.isoformat()}; "
                    "cannot verify."
                ),
                details=[
                    RuleResultDetail(
                        key=acct.account_ref,
                        message="Multiple AR total lines matched.",
                        values={
                ...
                human_action="Use a single AR total line or configure specific account refs.",
            )
        if total_matches:
        ...
        if not accounts_to_eval:
            return RuleResult(
                ...
                status=RuleStatus.NOT_APPLICABLE,
                severity=severity_for_status(RuleStatus.NOT_APPLICABLE),
                summary=f"No Accounts Receivable accounts found as of {ctx.period_end.isoformat()}.",
                human_action="Configure AR account refs or name match to enable this rule.",
            )
        ...
        if missing_refs:
            return RuleResult(
                ...
                severity=severity_for_status(RuleStatus.NEEDS_REVIEW),
                summary=(
                    f"Some configured AR accounts were missing from the Balance Sheet as of "
                ...
                human_action="Confirm AR account refs and ensure the Balance Sheet snapshot is complete.",
            )
        ...
        if summary_item is None or summary_item.amount is None:
            return RuleResult(
                ...
                severity=severity_for_status(RuleStatus.NEEDS_REVIEW),
                summary=(
                    f"Missing AR aging summary total for {ctx.period_end.isoformat()}; cannot verify."
                ),
                evidence_used=[summary_item] if summary_item else [],
                human_action="Provide the AR aging summary total as of period end.",
            )
        if detail_item is None or detail_item.amount is None:
            return RuleResult(
                ...
                severity=severity_for_status(RuleStatus.NEEDS_REVIEW),
                summary=(
                    f"Missing AR aging detail total for {ctx.period_end.isoformat()}; cannot verify."
                ),
                evidence_used=[detail_item] if detail_item else [],
                human_action="Provide the AR aging detail total as of period end.",
            )
        ...
        if cfg.require_evidence_as_of_date_match_period_end:
            if summary_item.as_of_date is None or summary_item.as_of_date != ctx.period_end:
                return RuleResult(
                    ...
                    severity=severity_for_status(RuleStatus.NEEDS_REVIEW),
                    summary=(
                        "AR aging summary as-of date is missing or does not match period end; cannot verify."
                    ),
                    evidence_used=[summary_item],
                    human_action="Provide the AR aging summary as of the period end date.",
                )
            if detail_item.as_of_date is None or detail_item.as_of_date != ctx.period_end:
                return RuleResult(
                    ...
                    severity=severity_for_status(RuleStatus.NEEDS_REVIEW),
                    summary=(
                        "AR aging detail as-of date is missing or does not match period end; cannot verify."
                    ),
                    evidence_used=[detail_item],
                    human_action="Provide the AR aging detail report as of the period end date.",
                )
        ...
        if diff_summary == 0 and diff_detail == 0:
            ...
            summary = f"AR aging totals reconcile to the Balance Sheet as of {ctx.period_end.isoformat()}."
        else:
            status = RuleStatus.FAIL
            severity = severity_for_status(status)
            summary = (
                f"AR aging totals do not reconcile to the Balance Sheet as of {ctx.period_end.isoformat()}."
            )

        human_action = None
        if status != RuleStatus.PASS:
            human_action = (
                "Reconcile the AR aging summary/detail totals to the Balance Sheet and resolve discrepancies."
            )

        details = [
            RuleResultDetail(
                key=ref,
                message="AR account included in Balance Sheet total.",
                values={
                    "account_name": name,
                    "period_end": ctx.period_end.isoformat(),
        ...
        details.append(
            RuleResultDetail(
                key="ar_aging_totals",
                message="AR aging totals compared to Balance Sheet total.",
                values={
                    "period_end": ctx.period_end.isoformat(),
                    "bs_total": str(bs_q),
    ...
    def _is_total_ar(self, name: str) -> bool:
        n = name.strip().lower()
        if "total" not in n:
            return False
