...
## Intent
Ensure AR aging summary and detail totals reconcile to the Balance Sheet AR total as of period end.

## Inputs (required)
- `RuleContext.period_end`
- `BalanceSheetSnapshot` at `period_end` (include “Total Accounts Receivable” line when possible)
- `EvidenceBundle` containing:
  - AR aging summary total
  - AR aging detail total
...
- `account_name_match` (default `accounts receivable`)
...
## Edge cases
- If a “Total Accounts Receivable” line exists, the rule uses it instead of summing accounts.
- If multiple AR accounts match by name, the rule sums them.
...
## Tests
- `src/backend/tests/rules_engine/test_bs_ar_subledger_reconciles.py`
